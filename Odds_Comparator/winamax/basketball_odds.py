"""
Scrape basketball odds from Winamax.

Sport ID : 2 (basketball)
Marchés  : Vainqueur (2-way), Total points (over/under), Handicap

Usage:
    python basketball_odds.py                   # tous les matchs
...
def scrape_all_basketball_odds(
    ...
    print(f'Found {len(matches_raw)} basketball matches.', file=sys.stderr)
    if not matches_raw:
        return []
...
def main():
    parser = argparse.ArgumentParser(description='Scrape Winamax basketball odds')
    ...
    with WinamaxClient(sport_id=SPORT_ID) as client:
        matches = scrape_all_basketball_odds(
            client,
