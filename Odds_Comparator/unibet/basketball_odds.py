"""
Scrape basketball odds from Unibet France.

Sport code : BASK
Marchés    : Vainqueur (2-way), Total points, Handicap, Quarts.
...
def scrape_basketball_odds(
    ...
    if league_ids is None:
        ...
        print(f'Found {len(leagues)} active basketball leagues.', file=sys.stderr)
    else:
...
def main():
    parser = argparse.ArgumentParser(description='Scrape Unibet France basketball odds')
    ...
    with UnibetClient() as client:
        matches = scrape_basketball_odds(
            client,
