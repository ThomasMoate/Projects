"""
Scrape football odds from Betclic France via gRPC-web.

Marchés : Résultat 1X2, Total buts, Handicap asiatique, Mi-Temps.

Usage:
    python football_odds.py                    # tous les matchs, catégorie Le Top
    python football_odds.py --all-categories   # toutes les catégories
...
def scrape_all_football(
    ...
    raw_matches = client.get_matches('football')
    print(f'Found {len(raw_matches)} football matches.', file=sys.stderr)
        ...
        try:
            if all_categories:
                markets_raw = client.get_all_match_markets_for_categories(
                    match_id, FOOTBALL_CATEGORIES, max_seconds=max_seconds
                )
            else:
...
def main():
    parser = argparse.ArgumentParser(description='Scrape Betclic France football odds')
    ...
    parser.add_argument('--category', default=None,
                        choices=list(FOOTBALL_CATEGORIES.keys()),
                        help='Fetch a specific category')
    args = parser.parse_args()

    with BetclicClient() as client:
        matches = scrape_all_football(
            client,
