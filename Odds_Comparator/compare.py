...
Usage :
    ...
    # Lance les scrapers automatiquement
...
def run_scraper(subdir: str, args: list[str], output_path: str) -> bool:
    script = _SCRIPT_DIR / subdir / 'tennis_odds.py'
    cmd = [sys.executable, str(script), '--output', output_path, '--prematch-only'] + args
    print(f'  Lancement {subdir}…', flush=True)
    result = subprocess.run(cmd, capture_output=True, text=True,
                            cwd=str(_SCRIPT_DIR / subdir))
    if result.returncode != 0:
        print(f'  ERREUR {subdir}: {result.stderr[:200]}', file=sys.stderr)
        return False
    return True
...
def main():
    ...
    if args.fetch:
        print('Récupération des cotes en cours…')
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            run_scraper('winamax', [], paths['winamax'])
            run_scraper('betclic', [], paths['betclic'])
            run_scraper('unibet',  [], paths['unibet'])
